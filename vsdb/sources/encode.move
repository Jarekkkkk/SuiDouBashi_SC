module suiDouBashi_vsdb::encode{
    ...
    public fun hex_encode(bytes: vector<u8>): vector<u8> {
    ...
    public fun hex_decode(bytes: vector<u8>): vector<u8> {
         let retval = vector::empty<u8>();
         let n = vector::length(&bytes);
    ...
    public fun base64_encode(bytes: vector<u8>): vector<u8>{
    ...
    public fun base64_decode(bytes: vector<u8>): vector<u8>{
        if (vector::length(&bytes) == 0){
            return b""
        };
        let retval = vector::empty<u8>();
